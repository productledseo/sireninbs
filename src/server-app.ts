...
app.get(["/api/alerts", "/api/alerts/"], async (req, res) => {
  ...
  try {
    ...
    if (source === "redalert") {
      recentAlerts = alerts.filter((alert: any) => alert.date >= twentyMinutesAgoTs).map(alert => ({
        cities: [alert.area],
        timestamp: alert.date,
        category: alert.threat
      }));
    } else if (source === "tzevaadom") {
      recentAlerts = alerts.filter((entry: any) => Array.isArray(entry) && entry.length >= 4 && entry[3] >= twentyMinutesAgoTs).map(entry => ({
        cities: entry[2],
        timestamp: entry[3],
        category: entry[1]
      }));
      ...
      recentAlerts = alerts.filter((alert: any) => {
        try {
          ...
          if (alert.alertDate && alert.alertDate.includes('.')) {
            ...
            alertTime = new Date(parseInt(year), parseInt(month) - 1, parseInt(day), parseInt(hour), parseInt(minute), parseInt(second));
          } else {
    ...
    const hasBeitShemesh = recentAlerts.some((alert: any) => 
      alert.cities.some((city: string) => city.includes("בית שמש") || city.toLowerCase().includes("beit shemesh"))
    );
    ...
    const hasJerusalem = recentAlerts.some((alert: any) => 
      alert.cities.some((city: string) => city.includes("ירושלים") || city.toLowerCase().includes("jerusalem"))
    );

    return res.json({ 
      status: hasJerusalem ? "siren expected" : "siren maybe",
      beitShemesh: true,
      jerusalem: hasJerusalem,
      source
    });
